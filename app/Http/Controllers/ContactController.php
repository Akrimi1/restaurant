...

{
...

    {
        ...
        return view('contact.index')->with('stores_name', $stores);
...

    {
        ...
        //return view('contact.index')->with('stores', $stores)->with("customFields", isset($html) ? $html : false);
 ...
 $contact = $this->contactRepository->where('restaurant_id',$user_id)->where('role_id','3')->get();
 ...
 $contact = DB::table('support')
 ->join('restaurants', 'restaurants.id', '=', 'support.stores')
 ->where('support.manager_id', $user_id)
 ->where('support.stores','!=', 'null')
 ->get();
