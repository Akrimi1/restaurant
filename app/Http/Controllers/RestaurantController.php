...
class RestaurantController extends Controller
{
    ...
    public function index(RestaurantDataTable $restaurantDataTable)
    {
        ...
        $manager_id = DB::table("restaurants")->pluck('managerId')
            ->first();
        // echo $manager_id; die;
    ...
    {
        //dd($request->all());
        $validator = \Validator::make($request->all() , ['name' => 'required|max:255', 'email' => 'required|email|max:255', 'subject' => 'required', 'bodymessage' => 'required']);
        if ($validator->fails())
        {
        ...
        {
            $message->to('your.ranisanisws1@gmail')
                ->subject('Subject of the message!');
        });
        return redirect('contact.index')
            ->with('status', 'You have successfully sent an email to the admin!');
    }
    ...
    {
        $user_id = Auth::user()->id;
        $has_restaurant = DB::table('user_restaurants')->select('restaurant_id', 'user_id', 'restaurants.name')
            ->join('restaurants', 'restaurants.id', '=', 'user_restaurants.restaurant_id')
        ...
        $user = $this
            ->userRepository
            ->getByCriteria(new ManagersCriteria())
            ->pluck('name', 'id');
        $drivers = $this
            ->userRepository
        ...
        if ($hasCustomField)
        {
            $customFields = $this
                ...
                ->findByField('custom_field_model', $this
                ->restaurantRepository
                ->model());
            $html = generateCustomField($customFields);
        }
        $last_restaurant = DB::table('restaurants')->get();

            $id = ($last_restaurant->last()->id) + 1;
            $str_id = strval($id);

            $store_id = "";
            if ($id <= 9) 
                $store_id = "KC00" . $str_id;
            else if ($id >= 10 && $id <= 99)
                $store_id = "KC0" . $str_id;
            else 
                $store_id = "KC" . $str_id;
        return view('restaurants.create')->with("customFields", isset($html) ? $html : false)->with("user", $user)->with("vendors_list", $vendors_list)->with("drivers", $drivers)->with("usersSelected", $usersSelected)->with("driversSelected", $driversSelected)->with('cuisine', $cuisine)->with('cuisinesSelected', $cuisinesSelected)->with('daysSelected', $daysSelected)->with('store_id', $store_id);
        //	}
    ...
    public function store(CreateRestaurantRequest $request)
    {
        $user_id = Auth::user()->id;
        $manager_id = $user_id;
        $manager_name = DB::table('users')->where('id', $manager_id)->pluck('name')
            ->first();
        //dd($manager_name);
        $has_restaurant = DB::table('user_restaurants')->select('restaurant_id', 'user_id', 'restaurants.name')
            ->join('restaurants', 'restaurants.id', '=', 'user_restaurants.restaurant_id')
        ...
        if (is_null($has_restaurant))
        {
            ...
            return redirect()->back()
                ->with('already has registered for restaurant');
        }
        else
        {
            $vendors = DB::table("vendors")->get();
            ...
            $user_table_id = DB::table('users')->select('id')
                ->where('email', $restaurant_email)->first();
            ...
            /*////////////$sm = DB::table("restaurants")->insert(["store_id"=> $store_id,"name" => $name, "email" => $email, "vendor_name" => "Supermarket", "description" => $description, "address" => $address, "latitude" => $latitude, "longitude" => $longitude, "phone" => $phone, "mobile" => $mobile, "information" => $information, "opening_time" => $opening_time, "closing_time" => $closing_time, "working_days" => $working_days,
            "restaurant_comission"=>$restaurant_comission,
            "admin_commission" => $admin_commission, "delivery_fee"=>$delivery_fee, "delivery_range"=>$delivery_range, "default_tax"=>$default_tax,
            "closed" => $closed, "active" => $active, "available_for_delivery" => $available_for_delivery, "created_at"=>date(), "updated_at"=>date()]);

            $restaurant_id = DB::table('restaurants')->select('id')
                ->where("email", $restaurant_email)->first();
            ...
            try
            {
                ...
                $restaurant->customFieldsValues()
                    ->createMany(getCustomFieldsValues($customFields, $request));
                if (isset($input['image']) && $input['image'])
                {
                    $cacheUpload = $this
                        ->uploadRepository
                        ->getByUuid($input['image']);
                    $mediaItem = $cacheUpload->getMedia('image')
                        ->first();
    ...
    public function show($id)
    {
        $this
            ->restaurantRepository
            ->pushCriteria(new RestaurantsOfUserCriteria(auth()
            ->id()));
        $restaurant = $this
            ->restaurantRepository
            ->findWithoutFail($id);
    ...
    public function edit($id)
    {
        ...
        if ($restaurant['active'] == 0)
        {
            $user = $this
                ->userRepository
                ->getByCriteria(new ManagersClientsCriteria())
                ->pluck('name', 'id');
        }
        else
        {
        ...
        $cuisine = $this
            ->cuisineRepository
            ->pluck('name', 'id');
        $usersSelected = $restaurant->users()
            ->pluck('users.id')
            ->toArray();
        $driversSelected = $restaurant->drivers()
            ->pluck('users.id')
            ->toArray();
        $cuisinesSelected = $restaurant->cuisines()
            ->pluck('cuisines.id')
            ->toArray();
        //   vendors------------field  //
        $vendors = DB::table("vendors")->get();
        ...
        $customFieldsValues = $restaurant->customFieldsValues()
            ->with('customField')
            ->get();
        $customFields = $this
            ->customFieldRepository
    ...
    {
        $all_restaurant = DB::table('restaurants')->get();
        //print_r($all_restaurant); die;
        return view("restaurants.invoice", array(
    ...
    public function createPDF(Request $request)
    {
        ...
        $test = view("restaurants.invoice_pdf", array(
            'restaurant' => $items
        ));
        $pdf = PDF::loadView($test)->setPaper('a4', 'landscape')
            ->setWarnings(false)
            ->save('invoice_pdf.pdf');
        //$pdf->render();
        return $pdf->download('invoice_pdf.pdf');
    }
    public function storeProducts(Request $request)
    {
        DB::table('products')->insert(['name' => $request->name, 'company' => $request->company, 'amount' => $request->amount, 'available' => $request->available, 'description' => $request->description]);
